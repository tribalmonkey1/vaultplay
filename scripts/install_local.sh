#!/bin/bash
# =============================================================================
# scripts/install_local.sh — AppImage builder for Arch Linux (local dev use)
#
# This is your convenience script for building + installing a VaultPlay
# AppImage on your own dev machine between releases. It is NOT what
# produces your published releases anymore — .github/workflows/release.yml
# + scripts/build_appimage.sh do that automatically on every `git push
# --tags`. Use this script when you just want to test a local build
...
cleanup() {
    ...
    if [ -d "${REPO_DIR}/.build_venv" ]; then
        rm -rf "${REPO_DIR}/.build_venv"
    fi
}
trap cleanup EXIT

echo ""
echo -e "${CYAN}  ⬡  VaultPlay AppImage Builder (local dev build)${NC}"
echo    "  ──────────────────────────────────────────"
echo ""
...
# Copy app source files — everything now lives under src/, not the repo root.
info "Copying app source files from src/..."
cp "${SRC_DIR}/"*.py      "${APPDIR}/usr/bin/"
cp "${SRC_DIR}/ui/"*.py   "${APPDIR}/usr/bin/ui/"
cp "${ASSET_DIR}/"*       "${APPDIR}/usr/bin/assets/" 2>/dev/null || true
# Bundled so SettingsView._load_app_version() can read it at runtime — see
...
cp "${REPO_DIR}/${APPIMAGE_NAME}" "${INSTALL_DIR}/${APPIMAGE_NAME}"
...
echo ""
echo    "  This installed a LOCAL test build only — it did not publish a"
echo    "  release. To publish a real release for the self-update checker"
echo    "  to find, tag and push instead:"
