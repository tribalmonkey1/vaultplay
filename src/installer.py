...
def _game_engine(game) -> Optional[str]:
    """
    Best-effort read of this game's engine name, for Launch Options'
    engine-detected argument checkboxes (see launch_options.normalize_engine).

    NOTE: metadata.py/db.py do not fetch or store an IGDB "engine" field
    yet — this is forward-compatible scaffolding so engine-args wiring
    works the moment that column exists, without another pass through
    every call site. Returns None today for every game, which just means
    the engine-args section stays hidden, exactly as if "engine unknown".
    """
    try:
        return game["engine"]
...
def _extract_launch_info(title: str, prefix_path: Path, exe_path: str,
    ...
    game_id, if given, folds this game's Launch Options (see launch_options.py)
    into whichever launch_cmd was produced above — env vars, GameMode wrap,
    and appended args are applied identically regardless of which path built
    the base command, since both end in the exe invocation with nothing
    after it. game_id=None (e.g. called before a game_id exists yet) skips
    this entirely and returns the base command untouched.

    Returns (launch_cmd, launch_cwd, icon_path) — all strings, any may be "".
    """
    is_proton = Path(wine_bin).name == "proton"

    def _finalize(cmd: str, cwd: str, icon: str) -> tuple:
        if game_id is not None:
            try:
                import launch_options as lo_mod
                options = lo_mod.get_effective_options(game_id)
                cmd = lo_mod.apply_to_launch_cmd(
                    cmd, options, is_proton=is_proton,
                    engine=lo_mod.normalize_engine(engine))
            except Exception as e:
                log.warning("Launch Options: could not apply overrides for "
                           "game %s — launching with base command. %s",
                           game_id, e)
        return cmd, cwd, icon
    ...
    for f in APPS_DIR.glob("*.desktop"):
        try:
            ...
            return _finalize(exec_, cwd, "")   # icon always "" — caller fills it in
        except Exception:
            continue
    ...
    return _finalize(launch_cmd, game_dir, "")
    ...
    if install_tag == "iso":
        ...
        if exe_path:
            ...
            launch_cmd, launch_cwd, _ = _extract_launch_info(
                ...
                game_id=game_id, engine=_game_engine(fresh_game))
...
# ── Cogwheel Menu actions ──────────────────────────────────────────────────
# Both of these operate on an EXISTING install — never create a new Wine
# prefix, never re-extract, never touch game files. They exist so the
# Cogwheel Menu (see game_detail.py) can change one thing about an install
# without re-running the full install pipeline.

def parse_wine_bin_from_cmd(launch_cmd: str) -> str:
    """
    Extract the Wine or Proton binary path/name from a launch command string
    like:
...
def regenerate_launch(game_id: int, proton_version: str) -> dict:
    """
    Re-point an existing install at a different Proton/Wine version:
    resolves the new wine_bin, rebuilds launch_cmd/launch_cwd via
    _extract_launch_info() (same logic run_install() uses), rewrites the
    .desktop file in place, and stores the new proton_version + launch
    fields on the installs row.

    Does NOT create a new prefix, re-run redists, or touch game files —
    the existing Wine prefix at the stored wine_prefix path is reused as-is.
    If that prefix has never been initialized for the new version's Wine
    build, the next launch will show whatever Wine's own first-run behavior
    is; this function only changes what gets launched, not the prefix state.

    Returns {"success": bool, "error": str|None, "launch_cmd": str}.
    """
    game = db.get_game(game_id)
    if not game or not game["is_installed"]:
        return {"success": False, "error": "Game is not installed", "launch_cmd": ""}

    exe_path = (game["exe_path"] or "").strip()
    if not exe_path or not Path(exe_path).exists():
        return {"success": False,
                "error": "No valid executable on record for this install — "
                         "set the executable path first.",
                "launch_cmd": ""}

    prefix_path = Path(game["wine_prefix"] or "")
    if not prefix_path or not prefix_path.exists():
        return {"success": False,
                "error": "Wine prefix not found on disk — set the prefix path first.",
                "launch_cmd": ""}

    import protondb as protondb_mod
    version_path = protondb_mod.get_version_path(proton_version)
    wine_bin = _resolve_wine_bin(version_path) if version_path else "wine"

    title = game["title"] or game["display_name"] or game["folder_name"]
    game_dir = str(Path(game["install_path"])) if game["install_path"] else str(Path(exe_path).parent)

    launch_cmd, launch_cwd, _ = _extract_launch_info(
        title, prefix_path, exe_path, wine_bin, game_dir,
        game_id=game_id, engine=_game_engine(game))

    icon_path = game["launch_icon"] or ""
    try:
        desktop_path = create_direct_desktop(title, launch_cmd, launch_cwd, icon_path)
    except Exception as e:
        log.error("regenerate_launch: failed to rewrite .desktop for game %d: %s", game_id, e)
        desktop_path = game["desktop_path"] or ""

    db.update_install_fields(
        game_id,
    ...
    log.info("regenerate_launch: game %d now launches via %s (%s)",
             game_id, wine_bin, proton_version or "(default)")
    return {"success": True, "error": None, "launch_cmd": launch_cmd}


def rerun_redists(game_id: int, redists: list, force: bool = True,
                  progress_cb: Optional[Callable] = None) -> dict:
    """
    Re-run winetricks against an existing install's Wine prefix with a
    (possibly edited) redist list, then persist that list on the installs
    row so the Cogwheel's "Edit Redistributables" dialog pre-checks the
    same set next time.

    force=True by default here (unlike the initial install flow) — this
    action exists specifically for the user to add a verb they realize is
    missing, or to force a clean reinstall of one that's misbehaving; a
    silent no-op skip on "already installed" would defeat the point of
    reaching for this action in the first place. Pass force=False to keep
    the normal skip-already-installed behavior instead.

    Returns {"success": bool, "error": str|None, "failed": list[str]}.
    Per the Cogwheel Menu spec, callers MUST check "failed" and show it —
    install_redists() already reports failures instead of swallowing them,
    and this call site needs to preserve that, not silently drop it.
    """
    game = db.get_game(game_id)
    if not game or not game["is_installed"]:
        return {"success": False, "error": "Game is not installed", "failed": []}
    ...
    failed = install_redists(prefix_path, redists, progress_cb, force=force)
    db.update_install_fields(game_id, redists=redists)
    log.info("rerun_redists: game %d — %d requested, %d failed",
             game_id, len(redists), len(failed))
    return {"success": True, "error": None, "failed": failed}
