...
# ── App version ────────────────────────────────────────────────────────────
# Single source of truth: the VERSION file at the app root, written by the
# release workflow from the git tag at build time (see
# .github/workflows/release.yml + scripts/build_appimage.sh, which copy it
# into AppDir/usr/bin/VERSION so it ships inside the AppImage). Falls back
# to "0.0.0-dev" when running straight from a source checkout with no
# VERSION file present — the normal dev workflow.

def _load_app_version() -> str:
    candidates = []
    appdir = _os.environ.get("APPDIR", "")
    if appdir:
        candidates.append(Path(appdir) / "usr" / "bin" / "VERSION")
    # Dev/source run: this file lives at src/ui/settings_view.py, so
    # parent.parent is src/ — VERSION isn't there, it's one level above
    # that at the repo root, hence parent.parent.parent below. Both are
    # checked since AppImage builds flatten ui/ alongside VERSION under
    # usr/bin/, while a source checkout keeps the repo-root VERSION file
...
class SettingsView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # About is always the last nav item — stored so app_update
        # notification clicks (see main_window._on_notification_navigate)
        # can jump straight to it via show_about() below.
        self._about_page_index = len(nav_defs) - 1
    ...
    def load_settings(self):
        ...
        # Refresh tags list
        if hasattr(self, "tags_active_layout"):
    ...
    def show_about(self):
        """Jump directly to the About page — used when an app_update
        notification is clicked (see main_window._on_notification_navigate)."""
        self._show_page(self._about_page_index)
    ...
    def _build_tags_page(self) -> QScrollArea:
        scroll, layout = self._scrollable_page("Tags")

        desc = QLabel(
            "Tags are shared across every game regardless of category. New "
            "tags are auto-imported from IGDB genres during metadata fetch, "
            "or added manually from a game's detail page. Rename a tag to "
            "relabel it everywhere it's used; delete a tag to remove it "
            "from every game and stop it from being auto-imported again."
        )
        desc.setFont(QFont("DM Sans", 11))
        desc.setStyleSheet(f"color: {COLORS['text_muted']}; padding-bottom: 12px;")
        desc.setWordWrap(True)
        ...
        layout.addWidget(SectionHeader("Active Tags"))

        self.tags_active_container = QWidget()
        self.tags_active_container.setStyleSheet("background: transparent;")
        self.tags_active_layout = QVBoxLayout(self.tags_active_container)
        self.tags_active_layout.setContentsMargins(0, 0, 0, 0)
        self.tags_active_layout.setSpacing(6)
        layout.addWidget(self.tags_active_container)

        layout.addSpacing(20)
        layout.addWidget(SectionHeader("Suppressed Tags"))

        suppressed_note = QLabel(
            "Deleted tags stay listed here so they can be brought back. "
            "Reinstating a tag does not retroactively re-add it to any "
            "game — it just allows it to be created again (manually, or "
            "via IGDB auto-import) going forward."
        )
        suppressed_note.setFont(QFont("DM Sans", 10))
        suppressed_note.setStyleSheet(f"color: {COLORS['text_muted']}; padding-bottom: 6px;")
        suppressed_note.setWordWrap(True)
        layout.addWidget(suppressed_note)

        self.tags_suppressed_container = QWidget()
        self.tags_suppressed_container.setStyleSheet("background: transparent;")
        self.tags_suppressed_layout = QVBoxLayout(self.tags_suppressed_container)
        self.tags_suppressed_layout.setContentsMargins(0, 0, 0, 0)
        self.tags_suppressed_layout.setSpacing(6)
        layout.addWidget(self.tags_suppressed_container)
    ...
    def _refresh_tags_list(self):
        # Active tags
        while self.tags_active_layout.count():
            item = self.tags_active_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()

        active = db.get_all_tags()
        if not active:
            empty = QLabel("No tags yet — tags appear here once auto-imported "
                           "from IGDB genres or added on a game's detail page.")
            empty.setFont(QFont("DM Sans", 11))
            empty.setStyleSheet(f"color: {COLORS['text_muted']};")
            empty.setWordWrap(True)
            self.tags_active_layout.addWidget(empty)
        else:
            for tag in active:
                self.tags_active_layout.addWidget(self._make_active_tag_row(tag))

        # Suppressed tags
        while self.tags_suppressed_layout.count():
            item = self.tags_suppressed_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()

        suppressed = db.get_suppressed_tags()
        if not suppressed:
            empty2 = QLabel("No suppressed tags.")
            empty2.setFont(QFont("DM Sans", 11))
            empty2.setStyleSheet(f"color: {COLORS['text_muted']};")
            self.tags_suppressed_layout.addWidget(empty2)
        else:
            for tag in suppressed:
                self.tags_suppressed_layout.addWidget(self._make_suppressed_tag_row(tag))

    def _make_active_tag_row(self, tag) -> QWidget:
        row = QWidget()
        row.setStyleSheet(f"""
            QWidget {{
                background: {COLORS['surface2']};
        ...
        name_edit.setStyleSheet(f"color: {COLORS['text']}; min-width: 160px;")
        name_edit.editingFinished.connect(
            lambda tid=tag["id"], e=name_edit: self._on_rename_tag(tid, e.text().strip())
        )
        row_l.addWidget(name_edit, 1)

        count = tag["usage_count"] or 0
        count_lbl = QLabel(f"{count} game{'s' if count != 1 else ''}")
        ...
        delete_btn.setFont(QFont("DM Sans", 10))
        ...
        delete_btn.clicked.connect(
            lambda _, tid=tag["id"], name=tag["name"], n=count: self._on_delete_tag(tid, name, n)
        )
        row_l.addWidget(delete_btn)

        return row

    def _make_suppressed_tag_row(self, tag) -> QWidget:
        row = QWidget()
        row.setStyleSheet(f"""
            QWidget {{
        ...
        reinstate_btn.setFont(QFont("DM Sans", 10))
        reinstate_btn.setStyleSheet(f"""
            QPushButton {{
                background: {COLORS['surface3']};
                border: 1px solid {COLORS['border']};
        ...
        """)
        reinstate_btn.clicked.connect(
            lambda _, tid=tag["id"]: self._on_reinstate_tag(tid)
        )
        row_l.addWidget(reinstate_btn)

        return row

    def _on_rename_tag(self, tag_id: int, new_name: str):
        if not new_name:
            self._refresh_tags_list()   # revert the blanked edit back to its old name
            return
        db.rename_tag(tag_id, new_name)
        self._refresh_tags_list()
        self.reload_requested.emit()

    def _on_delete_tag(self, tag_id: int, name: str, usage_count: int):
        from PyQt6.QtWidgets import QMessageBox
        msg = QMessageBox(self)
        msg.setWindowTitle("Delete Tag")
        msg.setText(f"Delete tag '{name}'?")
        if usage_count:
            msg.setInformativeText(
                f"This will remove it from {usage_count} game"
                f"{'s' if usage_count != 1 else ''}. It also won't be "
                "auto-imported again from IGDB genres until you reinstate "
                "it below. This cannot be undone.")
        else:
            msg.setInformativeText(
                "This tag isn't attached to any game currently. It won't "
                "be auto-imported again from IGDB genres until reinstated.")
    ...
    def _on_reinstate_tag(self, tag_id: int):
        db.reinstate_tag(tag_id)
        self._refresh_tags_list()
    ...
    def _build_appearance_page(self) -> QScrollArea:
        ...
        layout.addSpacing(24)
        layout.addWidget(SectionHeader("Search Scope"))

        search_scope_note = QLabel(
            "The library search box always matches the title. Optionally "
            "include these fields too — matching any enabled field shows the game."
        )
        search_scope_note.setFont(QFont("DM Sans", 11))
        search_scope_note.setStyleSheet(f"color: {COLORS['text_muted']}; padding-bottom: 4px;")
        search_scope_note.setWordWrap(True)
        layout.addWidget(search_scope_note)

        title_always_row = SettingsRow("Search Title", "Always searched — cannot be disabled")
        title_always_lbl = QLabel("✓ Always On")
        title_always_lbl.setFont(QFont("DM Sans", 10, QFont.Weight.Medium))
        title_always_lbl.setStyleSheet("color: #4ade80; background: transparent;")
        title_always_row.add_control(title_always_lbl)
        layout.addWidget(title_always_row)

        dev_search_toggle = SettingsToggle(
            db.get_setting("search_scope_developer", "true") == "true")
        dev_search_toggle.changed.connect(
            lambda v: self._save("search_scope_developer", "true" if v else "false"))
        self._make_setting_row(layout, "search_scope_developer",
                               "Search Developer",
                               "Include the developer field when matching search terms",
                               dev_search_toggle)

        pub_search_toggle = SettingsToggle(
            db.get_setting("search_scope_publisher", "true") == "true")
        pub_search_toggle.changed.connect(
            lambda v: self._save("search_scope_publisher", "true" if v else "false"))
        self._make_setting_row(layout, "search_scope_publisher",
                               "Search Publisher",
                               "Include the publisher field when matching search terms",
                               pub_search_toggle)

        layout.addSpacing(24)
        layout.addWidget(SectionHeader("Selection"))

        selection_note = QLabel(
            "Controls how you enter multi-select mode on the library grid, "
            "for acting on several games at once (favorite, hide, add to "
            "collection, set completion status, refresh metadata)."
        )
        selection_note.setFont(QFont("DM Sans", 11))
        selection_note.setStyleSheet(f"color: {COLORS['text_muted']}; padding-bottom: 4px;")
        selection_note.setWordWrap(True)
        layout.addWidget(selection_note)

        method_combo = QComboBox()
        for label, val in [("Long Press", "long_press"),
                            ("Modifier Key", "modifier"),
                            ("Both", "both")]:
            method_combo.addItem(label, val)
        idx = method_combo.findData(db.get_setting("selection_method", "both"))
        method_combo.setCurrentIndex(max(0, idx))
        self._make_setting_row(layout, "selection_method",
                               "Selection Method",
                               "How to enter multi-select mode on a game tile",
                               method_combo)

        long_press_ms_edit = QLineEdit(db.get_setting("selection_long_press_ms", "500"))
        long_press_ms_edit.setFont(QFont("DM Mono", 10))
        long_press_ms_edit.setFixedWidth(80)
        long_press_ms_edit.setStyleSheet(f"color: {COLORS['accent2']};")
        long_press_row = SettingsRow("Long Press Duration",
                                     "Milliseconds to hold before multi-select activates")
        long_press_row.add_control(long_press_ms_edit)
        layout.addWidget(long_press_row)
        self._widgets["selection_long_press_ms"] = long_press_ms_edit

        modifier_combo = QComboBox()
        for label, val in [("Ctrl", "ctrl"), ("Alt", "alt"), ("Meta", "meta")]:
            modifier_combo.addItem(label, val)
        idx2 = modifier_combo.findData(db.get_setting("selection_modifier_key", "ctrl"))
        modifier_combo.setCurrentIndex(max(0, idx2))
        modifier_row = SettingsRow("Modifier Key",
                                   "Held while clicking a tile to enter multi-select")
        modifier_row.add_control(modifier_combo)
        layout.addWidget(modifier_row)
        self._widgets["selection_modifier_key"] = modifier_combo

        def _save_long_press_ms():
            text = long_press_ms_edit.text().strip()
            if text.isdigit() and int(text) > 0:
                self._save("selection_long_press_ms", text)
            else:
                long_press_ms_edit.setText(db.get_setting("selection_long_press_ms", "500"))
        long_press_ms_edit.editingFinished.connect(_save_long_press_ms)

        modifier_combo.currentIndexChanged.connect(
            lambda: self._save("selection_modifier_key", modifier_combo.currentData()))

        def _update_selection_rows_visibility():
            method = method_combo.currentData()
            long_press_row.setVisible(method in ("long_press", "both"))
            modifier_row.setVisible(method in ("modifier", "both"))

        def _on_method_changed():
            self._save("selection_method", method_combo.currentData())
            _update_selection_rows_visibility()
    ...
    def _build_about_page(self) -> QScrollArea:
        ...
        layout.addWidget(SectionHeader("Updates"))

        if not update_check.is_running_as_appimage():
            appimage_note = QLabel(
                "Running from source, not a packaged AppImage — checking for "
                "updates still works, but downloading and restarting to update "
                "require the AppImage build."
            )
            appimage_note.setFont(QFont("DM Sans", 10))
            appimage_note.setStyleSheet(f"color: {COLORS['text_muted']}; padding-bottom: 6px;")
            appimage_note.setWordWrap(True)
            layout.addWidget(appimage_note)

        last_checked_row = SettingsRow(
            "Last Checked", "When VaultPlay last checked GitHub for a newer release")
        self.update_last_checked_lbl = QLabel("Never")
        self.update_last_checked_lbl.setFont(QFont("DM Mono", 9))
        self.update_last_checked_lbl.setStyleSheet(f"color: {COLORS['text_muted']};")
        last_checked_row.add_control(self.update_last_checked_lbl)
        layout.addWidget(last_checked_row)

        check_row = SettingsRow(
            "Check for Updates",
            "VaultPlay also checks silently in the background on every launch "
            "and shows a notification if a newer version is found.")
        self.check_updates_btn = ActionButton("Check for Updates")
        self.check_updates_btn.clicked.connect(self._on_check_updates_clicked)
        check_row.add_control(self.check_updates_btn)
        layout.addWidget(check_row)

        self.update_status_lbl = QLabel("")
        self.update_status_lbl.setFont(QFont("DM Mono", 9))
        self.update_status_lbl.setStyleSheet(f"color: {COLORS['text_muted']};")
        self.update_status_lbl.setWordWrap(True)
        layout.addWidget(self.update_status_lbl)

        update_action_row = QHBoxLayout()
        update_action_row.setContentsMargins(0, 4, 0, 12)
        update_action_row.setSpacing(10)

        self.download_update_btn = ActionButton("↓  Download Update")
        self.download_update_btn.clicked.connect(self._on_download_update_clicked)
        self.download_update_btn.hide()
        update_action_row.addWidget(self.download_update_btn)

        self.restart_update_btn = QPushButton("Restart to Update")
        self.restart_update_btn.setFont(QFont("Rajdhani", 12, QFont.Weight.Bold))
        self.restart_update_btn.setStyleSheet(accent_button_style())
        self.restart_update_btn.clicked.connect(self._on_restart_to_update_clicked)
        self.restart_update_btn.hide()
        update_action_row.addWidget(self.restart_update_btn)
        ...
        self._downloaded_appimage_path = None
        self._refresh_update_status_from_settings()
    ...
    # existing convention in this file (see _refresh_all_protondb / _start_redist_refresh
    # above). The silent launch-time check is a separate worker owned by
    # MainWindow (see main_window.UpdateCheckWorker) — this page only reflects
    # whatever the most recent check (either one) wrote to settings.

    def _refresh_update_status_from_settings(self):
        """
        Populate the Updates section from whatever the last check (auto or
        manual) recorded in settings, so opening this page shows current
        state without forcing a fresh network call.
        """
        last_at = db.get_setting("update_check_last_at", "")
        if last_at:
            import datetime
            try:
                dt = datetime.datetime.fromisoformat(last_at.replace("T", " ")[:19])
                delta = (datetime.datetime.utcnow() - dt).total_seconds()
                if delta < 60:
                    label = "Just now"
                elif delta < 3600:
                    label = f"{int(delta // 60)} minute(s) ago"
                elif delta < 86400:
                    label = f"{int(delta // 3600)} hour(s) ago"
                else:
                    label = f"{int(delta // 86400)} day(s) ago"
            except (ValueError, TypeError):
                label = last_at
            self.update_last_checked_lbl.setText(label)
        else:
            self.update_last_checked_lbl.setText("Never")

        tag  = db.get_setting("update_available_tag", "")
        date = db.get_setting("update_available_date", "")
        # Defensive re-check: only trust the cached "update available" flag if
        # it's still actually newer than this exact build — covers the case
        # where the cached value is stale from before a self-update already
        # replaced this AppImage with that version.
        if tag and update_check.is_update_available(self.APP_VERSION, tag):
            version = update_check.normalize_tag(tag)
            text = f"Update available: v{version}"
            if date:
                text += f"  (released {date})"
            self.update_status_lbl.setText(text)
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['accent']};")
            self.download_update_btn.setVisible(update_check.is_running_as_appimage())
            self.restart_update_btn.hide()
        else:
    ...
    def _on_update_check_result(self, release, current_version: str):
        """Called by MainWindow right after the silent launch-time check
        completes, so this page reflects it immediately if it's open. No-op
        if the widgets don't exist yet (page never built) — load_settings()
        already guards this the same way for the general reload path."""
        if hasattr(self, "update_last_checked_lbl"):
            self._refresh_update_status_from_settings()
    ...
    def _on_check_updates_clicked(self):
        ...
        self.update_status_lbl.setText("Checking GitHub for a newer release…")
        self.update_status_lbl.setStyleSheet(f"color: {COLORS['text_muted']};")
        self.download_update_btn.hide()
        self.restart_update_btn.hide()
        ...
        class _Worker(QThread):
            ...
            def run(self):
                self.done.emit(update_check.fetch_latest_release())

        self._update_worker = _Worker()
        self._update_worker.done.connect(self._on_manual_check_done)
        self._update_worker.start()

    def _on_manual_check_done(self, release):
        """
        Manual check ALWAYS shows the result, regardless of skipped_app_version
        — skipping only ever suppresses the launch-time notification (see
        update_check.py / main_window._on_update_check_done), never this page.
        No notification is created here either — manual checks are silent by
        design, per spec.
        """
        ...
        if update_check.is_update_available(self.APP_VERSION, release["tag"]):
            ...
            self.update_status_lbl.setText("You're up to date.")
            self.update_status_lbl.setStyleSheet("color: #4ade80;")
            self.download_update_btn.hide()
            self.restart_update_btn.hide()
            self.update_last_checked_lbl.setText("Just now")

    def _on_download_update_clicked(self):
        if self._download_worker and self._download_worker.isRunning():
            return

        url = db.get_setting("update_available_url", "")
        if not url:
            self.update_status_lbl.setText(
                "✗ No download URL on record — click Check for Updates again.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['danger']};")
            return

        appimage_path = update_check.current_appimage_path()
        if not appimage_path:
            self.update_status_lbl.setText("✗ Not running as an AppImage — can't self-update.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['danger']};")
            return

        dest = Path(appimage_path).parent / "VaultPlay-new.AppImage"
        self._downloaded_appimage_path = str(dest)
        ...
        class _DlWorker(QThread):
            progress = _Signal(int, int)   # downloaded_bytes, total_bytes
            done     = _Signal(bool, str)  # success, error_msg

            def run(self):
                ok, err = update_check.download_update(
                    url, dest, progress_cb=lambda d, t: self.progress.emit(d, t))
                self.done.emit(ok, err)

        self._download_worker = _DlWorker()
    ...
    def _on_download_progress(self, downloaded: int, total: int):
        ...
        if total:
            ...
            self.update_status_lbl.setText(f"Downloading update… {pct}%  ({mb_done:.1f} MB)")
        else:
            self.update_status_lbl.setText(f"Downloading update… {mb_done:.1f} MB")

    def _on_download_done(self, success: bool, error_msg: str):
        self.download_update_btn.setEnabled(True)
        self.download_update_btn.setText("↓  Download Update")

        if success:
            self.update_status_lbl.setText("✓ Download complete — ready to restart and update.")
            self.update_status_lbl.setStyleSheet("color: #4ade80;")
            self.download_update_btn.hide()
        ...
        msg.setText("The update download failed.")
        msg.setInformativeText(
            f"{error_msg}\n\n"
            "Retry (resumes from where it stopped), or cancel and discard "
        ...
        if msg.clickedButton() == retry_btn:
            ...
            try:
                if self._downloaded_appimage_path and Path(self._downloaded_appimage_path).exists():
                    Path(self._downloaded_appimage_path).unlink()
            except OSError as e:
                log.warning("Could not remove partial download: %s", e)
            self.update_status_lbl.setText("Download cancelled.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['text_muted']};")

    def _on_restart_to_update_clicked(self):
        """
        Launches the detached updater script (see update_check.launch_self_update)
        and quits the app. The script itself waits for this process to fully
        exit before renaming VaultPlay-new.AppImage over the running AppImage
        and relaunching it — see update_check.py's _UPDATER_SCRIPT for why
        that ordering guarantees only one instance ever runs at once.
        """
        if not self._downloaded_appimage_path or not Path(self._downloaded_appimage_path).exists():
            self.update_status_lbl.setText("✗ Downloaded file not found — download again.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['danger']};")
            return

        target = update_check.current_appimage_path()
        if not target:
            self.update_status_lbl.setText("✗ Not running as an AppImage — can't self-update.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['danger']};")
            return

        ok = update_check.launch_self_update(self._downloaded_appimage_path, target)
        if not ok:
            self.update_status_lbl.setText(
                "✗ Could not start the updater — try again, or update manually.")
            self.update_status_lbl.setStyleSheet(f"color: {COLORS['danger']};")
            return
