    ...
    try:
        ...
        if rar:
            ...
            return {"file_type": "rar",
                    ...
                    # names is None means every peek method (rarfile, unrar,
                    # 7z binary) failed on a RAR that DOES exist on disk —
                    # a real, distinct condition from "empty archive", and
                    # the signal Notifications' corrupted_archive type
                    # surfaces to the user (see _register_game() below).
                    "corrupted": names is None}
        ...
        # Loose files — nothing to peek, so never "corrupted" by this definition
        ...
        return {"file_type": "loose", "install_tag": "portable",
                "archive_name": None, "size_bytes": 0, "corrupted": False}
...
def _register_game(folder_path: Path, category: str,
    ...
    try:
        ...
        # Notifications — corrupted_archive: an archive is present but every
        # peek method failed on it (see classify_folder() above). Collected
        # here regardless of new-vs-updated so a corrupted archive is
        # flagged on every scan pass that still finds it broken, not just
        # the first. db.add_notification()'s own dedup (skip if an unread
        # corrupted_archive notification already exists for this game)
        # keeps a rescan from spamming duplicates while it stays unread.
        if info.get("corrupted"):
            counters.setdefault("corrupted_games", []).append({
                "game_id":     game_id,
                "folder_name": folder_name,
                "nas_path":    str(folder_path),
            })
...
def scan_nas(nas_path: str, progress_callback=None) -> dict:
    ...
    return {
        ...
        # Games found this scan whose primary archive exists but couldn't be
        # read by any peek method — see classify_folder()/_register_game()
        # above. Used by MainWindow to fire Notifications' corrupted_archive
        # entries after the scan completes.
        "corrupted_games": counters.get("corrupted_games", []),
    }
