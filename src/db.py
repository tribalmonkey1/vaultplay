    ...
    with get_connection() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS game_state (
                ...
                launch_options       TEXT,
                -- Per-Game Notes feature: plain freetext, unlimited length.
                -- NULL = no notes. See get_notes()/set_notes() below.
                notes                TEXT
            );
            ...
            -- notifications: bell-icon feed in the library header. game_id is NULL for
            -- system-level notifications (scan_summary and the grouped version_update
            -- row). See add_notification() / upsert_version_update_notification() below
            -- for the dedup rules per type. No _migrate_db() entry needed — CREATE TABLE
            -- IF NOT EXISTS is sufficient since this is a brand-new table.
            ...
            -- collections / collection_games: user-created ordered lists of
            -- games (Collections / Playlists feature). ON DELETE CASCADE on
            -- both FKs means removing a game from the library automatically
            -- drops it from every collection, and deleting a collection
            -- drops all of its game associations — neither ever touches the
            -- games table itself.
            CREATE TABLE IF NOT EXISTS collections (
                id                  INTEGER PRIMARY KEY AUTOINCREMENT,
                name                TEXT NOT NULL,
                sort_order          INTEGER DEFAULT 0,
                game_order_locked   INTEGER DEFAULT 0
            );
            ...
            -- tags / game_tags / tag_renames: user-defined + IGDB-auto-imported
            -- tags. Global pool across all games/categories. See Notion →
            -- Features → Fully Planned → Tags for full spec.
            ...
            CREATE TABLE IF NOT EXISTS tag_renames (
...
def _init_default_settings():
    defaults = {
        ...
        "search_scope_developer": "true",   # Search Scope Expansion — dev field toggle
        "search_scope_publisher": "true",   # Search Scope Expansion — publisher field toggle
        ...
        "update_available_tag":         "",  # raw GitHub tag of the latest known newer release, e.g. "v0.4.0"
        "update_available_date":        "",  # that release's human-formatted date, e.g. "July 17, 2026"
        "update_available_url":         "",  # direct .AppImage asset download URL for that release
        # Collections / Playlists — Lock A (sidebar collection order).
        # Lock B (per-collection game order) lives on the collections row
        # itself (game_order_locked column), not here — see the collections
        # table. Both locks default UNLOCKED per spec.
        "collections_sidebar_locked":   "false",
        # Multi-Select & Bulk Tile Actions — how a tile enters multi-select.
        # 'long_press' | 'modifier' | 'both'. selection_long_press_ms only
        # matters when long_press is in effect; selection_modifier_key only
        # matters when modifier is in effect. See ui/library_view.py's
        # LibraryView._selection_settings()-style helpers and
        # ui/settings_view.py's Appearance → Selection section.
...
def _migrate_db():
    ...
    with get_connection() as conn:
        ...
        for col, sql in [
            ...
            # Cogwheel Menu additions:
            # proton_version — the version key (e.g. "ge-proton-10.5") last used to
            # launch this game, so "Change Proton Version" has something to show as
            # current and installer.regenerate_launch() knows what to resolve from.
            # Blank for installs made before this existed — falls back to
            # default_proton_version the same way the original install did.
            ("proton_version", "ALTER TABLE installs ADD COLUMN proton_version TEXT DEFAULT ''"),
            # redists — JSON list of winetricks verbs last (re)installed into this
            # prefix, so the Cogwheel's "Edit Redistributables" dialog has something
            # to pre-check instead of starting from steamdb's auto-detect every time.
            ("redists", "ALTER TABLE installs ADD COLUMN redists TEXT DEFAULT '[]'"),
        ]:
        ...
        for col, sql in [
            ...
            # dump, so a future change to a static default doesn't require
            # rewriting every game's stored row.
            ("launch_options",   "ALTER TABLE game_state ADD COLUMN launch_options TEXT"),
            # Per-Game Notes feature — plain freetext, unlimited length.
            # NULL = no notes (never an empty string — saving an emptied
            # field clears back to NULL, see set_notes()). Survives
            # uninstall like every other game_state column.
            ("notes",            "ALTER TABLE game_state ADD COLUMN notes TEXT"),
        ]:
...
# ── Cogwheel Menu helpers ─────────────────────────────────────────────────────

_INSTALL_UPDATABLE_COLS = frozenset({
    "install_path", "wine_prefix", "install_method", "exe_path", "game_path",
    "launcher_type", "desktop_path", "script_path", "launch_cmd", "launch_cwd",
    "launch_icon", "proton_version", "redists",
})


def update_install_fields(game_id: int, **fields):
    """
    Partial-update one or more columns on the `installs` row for a game.
    Only the field(s) passed are written — matching set_save_paths() /
    set_installed_version()'s partial-update contract elsewhere in this file.
    `redists` may be passed as a list (auto-serialized to JSON) or a raw string.
    No-op if there's no installs row for this game (nothing to update yet) or
    if no recognized fields were passed.

    Used by the Cogwheel Menu: changing Proton version, editing the
    redistributables list, and manually filling in exe/prefix/install paths
    for a game VaultPlay didn't detect (manual registration warning state)
    all go through this single call site.
    """
    updates = []
    params  = []
    for key, value in fields.items():
        if key not in _INSTALL_UPDATABLE_COLS:
            continue
        if key == "redists" and isinstance(value, (list, tuple)):
...
def get_install_redists(game_id: int) -> list:
    """Return the stored redists list for a game's install, or [] if unset/unparseable."""
    with get_connection() as conn:
        row = conn.execute(
            "SELECT redists FROM installs WHERE game_id=?", (game_id,)
        ).fetchone()
    if not row or not row["redists"]:
        return []
    try:
        data = json.loads(row["redists"])
        return data if isinstance(data, list) else []
    except (json.JSONDecodeError, TypeError):
        return []


def register_manual_install(game_id: int, exe_path: str = "",
                            wine_prefix: str = "", install_path: str = "") -> None:
    """
    Register a game as installed outside VaultPlay's own install flow (e.g.
    installed by hand, or via Lutris/Steam directly). Creates the `installs`
    row with install_method='manual' if one doesn't already exist, so
    is_installed becomes true and the Cogwheel Menu's warning state can be
    used to fill in / correct the paths afterward.

    Blank fields are allowed — this can be called with nothing filled in yet
    just to flip is_installed on, matching the Cogwheel spec's "game IS
    installed but app doesn't detect it" case where the user registers first
    and fills in details after.
    """
    with get_connection() as conn:
        existing = conn.execute(
            "SELECT id FROM installs WHERE game_id=?", (game_id,)
        ).fetchone()
    if existing:
        update_install_fields(
            game_id,
            exe_path=exe_path, wine_prefix=wine_prefix, install_path=install_path,
        )
        return
    record_install(
        game_id=game_id,
...
# ── Per-Game Notes helpers ──────────────────────────────────────────────────────
# Plain freetext, unlimited length, stored in game_state.notes. NULL = no
# notes at all. Survives uninstall — game_state is never deleted when a
# game is uninstalled, only when the game itself is removed from the NAS
# and its games row is deleted (cascades).

def get_notes(game_id: int) -> Optional[str]:
    """Return the stored notes text for a game, or None if never set."""
    with get_connection() as conn:
        row = conn.execute(
            "SELECT notes FROM game_state WHERE game_id=?", (game_id,)
        ).fetchone()
    return row["notes"] if row else None


def set_notes(game_id: int, notes: Optional[str]):
    """
    Save notes text for a game. An empty/whitespace-only string is stored
    as NULL (clears any existing notes) rather than as an empty string —
    this keeps "no notes" a single, unambiguous state matching
    get_notes()'s None return and the placeholder-text UI behavior.
    """
    value = notes.strip() if notes else None
    value = value if value else None
    with get_connection() as conn:
        conn.execute("""
            INSERT INTO game_state (game_id, notes)
            VALUES (?, ?)
            ON CONFLICT(game_id) DO UPDATE SET
...
# Storage contract: game_state.launch_options holds ONLY the keys that differ
# from launch_options.py's static/computed defaults — never a full snapshot.
# NULL/empty column = no overrides at all. See launch_options.py for the
# default values, merge logic, and command-construction that consumes this.

def get_launch_option_overrides(game_id: int) -> dict:
    """
    Return the raw stored overrides dict for a game (NOT merged with
    defaults — see launch_options.get_effective_options() for that).
    Returns {} if the game has no overrides stored yet, or if the stored
    JSON is unparseable (never raises — a corrupt blob should fall back to
    defaults, not crash the caller).
    """
    with get_connection() as conn:
        row = conn.execute(
            "SELECT launch_options FROM game_state WHERE game_id=?", (game_id,)
        ).fetchone()
    if not row or not row["launch_options"]:
        return {}
    try:
        data = json.loads(row["launch_options"])
        return data if isinstance(data, dict) else {}
    except (json.JSONDecodeError, TypeError):
        return {}


def set_launch_option_overrides(game_id: int, overrides: dict):
    """
    Store the overrides dict for a game, replacing whatever was there
    before (callers pass the full diff-from-defaults dict — this is not a
    partial-update merge like update_install_fields()). Passing an empty
    dict stores NULL, restoring the "no overrides" state rather than an
    empty-but-present '{}' string, so get_launch_option_overrides() and a
    fresh game read the same way.
    """
    payload = json.dumps(overrides) if overrides else None
    with get_connection() as conn:
        conn.execute("""
            INSERT INTO game_state (game_id, launch_options) VALUES (?, ?)
            ON CONFLICT(game_id) DO UPDATE SET
...
#
# Five types: version_update (grouped, game_id=NULL, update-in-place — see
# upsert_version_update_notification()), protondb_change / save_backup_failed /
# corrupted_archive (individual per game, deduped against unread rows of the
# same type+game), scan_summary (system-level, always a fresh row — a
# point-in-time event with no "same as before" to compare against).
#
# Read vs dismissed are separate concepts (per spec): mark_all_read() just
# flips is_read so the badge resets while the row still shows in the panel;
# dismiss_notification() actually deletes the row.

def add_notification(type: str, title: str, message: str,
                     game_id: Optional[int] = None, dedup: bool = True) -> Optional[int]:
    """
    Insert a notification row.

    dedup=True (default): only inserts if there's no existing UNREAD notification
    of the same type for the same game_id — this is what keeps protondb_change,
    save_backup_failed, and corrupted_archive from piling up duplicates while an
    existing one is still unread. Returns that existing row's id instead of
    inserting a new one.

    dedup=False: always inserts a fresh row — used for scan_summary, which is a
    point-in-time event with no natural "same as before" to compare against.

    Not used for the grouped version_update type — see
    upsert_version_update_notification() below for that update-in-place case.

    Returns the notification id (existing or newly created).
    """
    with get_connection() as conn:
        if dedup:
            # "game_id IS ?" (not "=") so this also matches correctly when
            # game_id is None (system-level notifications) — SQLite's "=" never
            # matches NULL, "IS" does.
            existing = conn.execute(
                "SELECT id FROM notifications WHERE type=? AND is_read=0 AND game_id IS ?",
                (type, game_id)
            ).fetchone()
            if existing:
                return existing["id"]
        cur = conn.execute("""
            INSERT INTO notifications (game_id, type, title, message, is_read)
            VALUES (?, ?, ?, ?, 0)
        """, (game_id, type, title, message))
        return cur.lastrowid


def upsert_version_update_notification(game_ids: list) -> None:
    """
    Grouped version_update notification — ONE aggregate row ("N games have
    version updates available"), game_id=NULL, updated in place rather than
    duplicated. Per spec: "only update if the set of games with updates
    actually changed."

    Since the notifications table itself carries no fingerprint column, the
    last-seen game-id set is tracked in a settings key
    (notif_version_update_ids) — same pattern as version_check_last_run_at
    elsewhere in this file for small pieces of cross-session state that don't
    warrant their own column.

    No-ops entirely if the set is identical to last time (including staying
    empty). If the set is newly empty (no games have updates anymore), any
    existing grouped notification is left as history rather than deleted —
    this function only ever creates or updates, never removes.
    """
    ids_sorted = sorted(set(game_ids))
    key = "notif_version_update_ids"
    try:
        prev_ids = sorted(json.loads(get_setting(key, "[]")))
    except (json.JSONDecodeError, TypeError, ValueError):
        prev_ids = []

    if ids_sorted == prev_ids:
        return

    set_setting(key, json.dumps(ids_sorted))
    ...
    message = f"{n} game{'s' if n != 1 else ''} have version updates available."

    with get_connection() as conn:
        existing = conn.execute("""
            SELECT id FROM notifications
            WHERE type='version_update' AND game_id IS NULL
            ORDER BY created_at DESC LIMIT 1
        """).fetchone()
        if existing:
            conn.execute("""
                UPDATE notifications
                SET title=?, message=?, is_read=0, created_at=datetime('now')
                WHERE id=?
            """, (title, message, existing["id"]))
        else:
            conn.execute("""
                INSERT INTO notifications (game_id, type, title, message, is_read)
                VALUES (NULL, 'version_update', ?, ?, 0)
            """, (title, message))


def get_notifications(limit: int = 50) -> list:
    """Return notifications newest-first, most recent `limit` rows."""
    with get_connection() as conn:
        return conn.execute("""
...
def get_notification_by_id(notification_id: int) -> Optional[sqlite3.Row]:
    """
    Return a single notifications row by id, or None.

    Used when a caller needs to know a notification's type/title before
    dismissing it — specifically AppImage Self-Update Check's app_update
    type, where dismissing the notification must record that version as
    skipped_app_version (see ui/library_view.py's NotificationPanel._on_dismiss).
    """
    with get_connection() as conn:
        return conn.execute(
...
def mark_all_read() -> None:
    """Flip every unread notification to read — badge resets to 0. Rows are
    NOT deleted; see dismiss_notification()/clear_all_notifications() for
    that. Called when the notification dropdown panel opens."""
    with get_connection() as conn:
        conn.execute("UPDATE notifications SET is_read=1 WHERE is_read=0")


def dismiss_notification(notification_id: int) -> None:
    """Permanently remove one notification (the panel's per-row ✕ button)."""
    with get_connection() as conn:
        conn.execute("DELETE FROM notifications WHERE id=?", (notification_id,))


def clear_all_notifications() -> None:
    """Permanently remove every notification (the panel's 'Clear all' button)."""
    with get_connection() as conn:
        conn.execute("DELETE FROM notifications")


# ── Collections / Playlists ───────────────────────────────────────────────────
# Spec: Notion → Features → Fully Planned → Collections / Playlists.
#
# Two locks, both default UNLOCKED:
#   Lock A (collections.sort_order, toggled via the collections_sidebar_locked
#           setting) — sidebar collection ordering.
#   Lock B (collections.game_order_locked, per-collection)
#     — in-collection game ordering.
#
# New collections are always appended to the bottom of the sidebar list;
# new games added to a collection are always appended to the end of that
# collection's game order — the curated order is never disrupted by an
# addition (see add_game_to_collection() / create_collection()).

def get_collections() -> list:
    """Return all collections ordered by sidebar sort_order, for sidebar display."""
    with get_connection() as conn:
        return conn.execute(
...
def get_collection(collection_id: int) -> Optional[sqlite3.Row]:
    with get_connection() as conn:
        return conn.execute(
...
def create_collection(name: str) -> int:
    """Create a new collection, appended to the end of the sidebar order —
    'new collections appear at the bottom by default' per spec."""
    with get_connection() as conn:
        row = conn.execute(
            "SELECT COALESCE(MAX(sort_order), -1) AS m FROM collections"
        ).fetchone()
        next_order = (row["m"] if row and row["m"] is not None else -1) + 1
        cur = conn.execute(
...
def create_collection_with_game(name: str, game_id: int) -> int:
    """'+ New Collection…' inline creation from the Add to Collection
    submenu — creates the collection and adds the game in one step."""
    collection_id = create_collection(name)
    add_game_to_collection(collection_id, game_id)
    return collection_id


def rename_collection(collection_id: int, name: str):
    name = name.strip()
    if not name:
        return
    with get_connection() as conn:
...
def delete_collection(collection_id: int):
    """Delete a collection. Cascades to collection_games only — games
    themselves are never removed from the library ('Games will not be
    removed from your library' per the confirmation dialog copy)."""
    with get_connection() as conn:
        conn.execute("DELETE FROM collections WHERE id=?", (collection_id,))


def set_collection_game_order_locked(collection_id: int, locked: bool):
    """Lock B toggle — per-collection game-order lock."""
    with get_connection() as conn:
        conn.execute(
            "UPDATE collections SET game_order_locked=? WHERE id=?",
            (1 if locked else 0, collection_id)
        )


def reorder_collections(ordered_ids: list):
    """Persist a new sidebar order (Lock A) — ordered_ids is the FULL list
    of collection ids in their new display order."""
    with get_connection() as conn:
        for i, cid in enumerate(ordered_ids):
            conn.execute(
                "UPDATE collections SET sort_order=? WHERE id=?", (i, cid)
            )


def get_collection_game_count(collection_id: int) -> int:
    """Live count for the sidebar's game-count badge — includes hidden
    games (the badge reflects true membership; hidden games are only
    excluded from the collection's rendered tile view, per spec)."""
    with get_connection() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS n FROM collection_games WHERE collection_id=?",
            (collection_id,)
        ).fetchone()
        return row["n"] if row else 0


def get_collection_ids_for_game(game_id: int) -> set:
    """Return the set of collection_ids this game currently belongs to —
    used to render checkmarks in the 'Add to Collection' submenu."""
    with get_connection() as conn:
        rows = conn.execute(
...
def add_game_to_collection(collection_id: int, game_id: int):
    """Append a game to the end of a collection's sort_order. No-op if the
    game is already a member (this is what makes the submenu's 'click an
    unchecked collection to add' safe to call blindly)."""
    with get_connection() as conn:
        existing = conn.execute(
        ...
        if existing:
            return
        row = conn.execute(
            "SELECT COALESCE(MAX(sort_order), -1) AS m FROM collection_games "
            "WHERE collection_id=?",
            (collection_id,)
        ).fetchone()
        next_order = (row["m"] if row and row["m"] is not None else -1) + 1
        conn.execute(
            "INSERT INTO collection_games (collection_id, game_id, sort_order) "
            "VALUES (?, ?, ?)",
            (collection_id, game_id, next_order)
        )
...
def get_game_ids_in_collection(collection_id: int) -> list:
    """Return game_ids in this collection's custom sort_order (ascending) —
    the authoritative Custom Order used when viewing the collection."""
    with get_connection() as conn:
        rows = conn.execute(
...
def reorder_games_in_collection(collection_id: int, ordered_game_ids: list):
    """Persist a new in-collection game order (Lock B). Caller passes the
    FULL new order for this collection."""
    with get_connection() as conn:
        for i, gid in enumerate(ordered_game_ids):
            conn.execute(
                "UPDATE collection_games SET sort_order=? "
                "WHERE collection_id=? AND game_id=?",
                (i, collection_id, gid)
            )


def get_collections_for_tile_menu() -> list:
    """Lightweight (id, name) list for building the 'Add to Collection'
    submenu — avoids pulling sort_order/game_order_locked the menu doesn't
    need."""
    with get_connection() as conn:
        return conn.execute(
            "SELECT id, name FROM collections ORDER BY sort_order, id"
        ).fetchall()


# ── Tags ──────────────────────────────────────────────────────────────────
# Spec: Notion → Features → Fully Planned → Tags.
#
# Global pool: one "horror" tag shared across every game regardless of
# category. Sources: auto-imported from IGDB genres during metadata fetch
# (see metadata.sync_tags_from_genres()), or user-created via the game
# detail tag input / metadata editor.

def _normalize_tag_name(name: str) -> str:
    return (name or "").strip()


def resolve_tag_rename(name: str) -> str:
    """Follow tag_renames to the canonical name for a tag, if one exists."""
    name = _normalize_tag_name(name)
    with get_connection() as conn:
        row = conn.execute(
            "SELECT canonical FROM tag_renames WHERE original=?", (name,)
        ).fetchone()
    return row["canonical"] if row else name


def get_tag_by_name(name: str) -> Optional[sqlite3.Row]:
    name = _normalize_tag_name(name)
    with get_connection() as conn:
        return conn.execute("SELECT * FROM tags WHERE name=?", (name,)).fetchone()


def get_or_create_tag(name: str, *, auto_import: bool = False) -> Optional[sqlite3.Row]:
    """
    Resolve a tag by canonical name (via tag_renames), creating it if it
    doesn't exist yet.

    auto_import=True (IGDB genre import path): if the resolved tag is
    suppressed (user-deleted), returns None and creates nothing — per spec,
    "future auto-imports that would create this tag are silently blocked."
    Never un-suppresses a tag; only Settings → Tags → Reinstate does that.

    auto_import=False (user explicitly typed/picked a tag): a suppressed
    tag is still returned/attachable rather than blocked — the user typing
    the exact name of a previously-deleted tag reads as a deliberate
    re-creation, not an auto-import resurrection. This does NOT clear the
    suppressed flag itself, so it stays out of future auto-import until
    explicitly reinstated. Undecided edge case in the spec — revisit if
    this proves surprising in practice.
    """
    canonical = resolve_tag_rename(name)
    if not canonical:
        return None
    with get_connection() as conn:
        row = conn.execute("SELECT * FROM tags WHERE name=?", (canonical,)).fetchone()
        if row:
            if auto_import and row["suppressed"]:
                return None
            return row
        conn.execute("INSERT INTO tags (name, suppressed) VALUES (?, 0)", (canonical,))
        return conn.execute("SELECT * FROM tags WHERE name=?", (canonical,)).fetchone()


def add_tag_to_game(game_id: int, tag_id: int):
    with get_connection() as conn:
        conn.execute(
...
def remove_tag_from_game(game_id: int, tag_id: int):
    """Removes only this game's association. The tag itself still exists
    globally — see delete_tag() to remove a tag from every game."""
    with get_connection() as conn:
        conn.execute(
...
def get_tags_for_game(game_id: int) -> list:
    with get_connection() as conn:
        return conn.execute("""
            SELECT t.* FROM tags t
            JOIN game_tags gt ON gt.tag_id = t.id
            WHERE gt.game_id = ? AND t.suppressed = 0
            ORDER BY t.name COLLATE NOCASE
        """, (game_id,)).fetchall()


def get_all_tags(include_suppressed: bool = False) -> list:
    """Non-suppressed tags with a live usage count, for the sidebar and
    Settings → Tags. include_suppressed=True is used by the Settings page's
    active-tags list rendering path (it queries suppressed separately via
    get_suppressed_tags() below, so this flag is mostly for completeness)."""
    where = "" if include_suppressed else "WHERE t.suppressed = 0"
    with get_connection() as conn:
        return conn.execute(f"""
            SELECT t.*, COUNT(gt.game_id) AS usage_count
            FROM tags t
...
def get_suppressed_tags() -> list:
    with get_connection() as conn:
        return conn.execute(
            "SELECT * FROM tags WHERE suppressed=1 ORDER BY name COLLATE NOCASE"
        ).fetchall()


def rename_tag(tag_id: int, new_name: str):
    """
    Global rename — updates the tag's own name and remembers the mapping so
    a future auto-import of the OLD name resolves to the new canonical name
    instead of creating a duplicate. Games keeping the tag need no change
    (game_tags references tag_id, never name).
    """
    new_name = _normalize_tag_name(new_name)
    if not new_name:
        return
    with get_connection() as conn:
        row = conn.execute("SELECT name FROM tags WHERE id=?", (tag_id,)).fetchone()
        if not row or row["name"] == new_name:
            return
        old_name = row["name"]
        conn.execute("UPDATE tags SET name=? WHERE id=?", (new_name, tag_id))
        conn.execute("""
            INSERT INTO tag_renames (original, canonical) VALUES (?, ?)
            ON CONFLICT(original) DO UPDATE SET canonical = excluded.canonical
        """, (old_name, new_name))
        # Re-point any earlier rename chain that resolved TO the old name —
        # otherwise a rename-of-a-rename would leave a stale intermediate
        # canonical value in tag_renames.
        conn.execute(
            "UPDATE tag_renames SET canonical=? WHERE canonical=?",
            (new_name, old_name)
        )
...
def delete_tag(tag_id: int) -> int:
    """
    Removes this tag from every game and marks it suppressed so future
    auto-imports don't silently recreate it. Returns the number of games it
    was removed from, for the confirmation prompt's game count.
    """
    with get_connection() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS n FROM game_tags WHERE tag_id=?", (tag_id,)
        ).fetchone()
        count = row["n"] if row else 0
        conn.execute("DELETE FROM game_tags WHERE tag_id=?", (tag_id,))
        conn.execute("UPDATE tags SET suppressed=1 WHERE id=?", (tag_id,))
        return count


def reinstate_tag(tag_id: int):
    """Un-suppresses a tag so it can be auto-created again on future
    metadata fetches. Does NOT retroactively re-add it to any game."""
    with get_connection() as conn:
        conn.execute("UPDATE tags SET suppressed=0 WHERE id=?", (tag_id,))


def get_games_with_all_tags(tag_ids: list) -> set:
    """
    Group C tag filter — AND logic, per spec: a game must have ALL of
    tag_ids to match. Empty input returns an empty set; callers should
    treat 'no tags selected' as 'no tag filter' and skip calling this.
    """
    if not tag_ids:
        return set()
