...
def find_version_value_for_binary(wine_bin: str) -> Optional[str]:
    """
    Reverse lookup: given a resolved Wine/Proton binary path or bare command
    (as pulled from a stored launch_cmd — see installer.parse_wine_bin_from_cmd),
    return the installed version's 'value' key it belongs to, or None if it
    doesn't match anything currently detected.

    Used to default the Cogwheel Menu's Proton/Wine version picker to
    whatever a game is ACTUALLY running with right now, for installs made
    before installs.proton_version existed (so that column is blank) —
    without this, the picker has nothing to go on and silently defaults to
    whatever sorts first (System Wine), which is misleading since it looks
    like a real answer instead of "unknown".
    """
    if not wine_bin:
        return None
    if wine_bin == "wine":
        # Bare "wine" with no path at all — only meaningful match is the
        # system-wine entry scan_installed_versions() adds.
        for v in _get_cached_installed():
            if v["value"] == "system-wine":
                return "system-wine"
        return None
    try:
        target = Path(wine_bin).resolve()
    except OSError:
        target = Path(wine_bin)
    for v in _get_cached_installed():
        vp = Path(v["path"])
        try:
            vp = vp.resolve()
            ...
            pass
        if target == vp or str(target).startswith(str(vp) + os.sep):
            return v["value"]
    return None
